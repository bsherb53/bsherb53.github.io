...
;(async () => {
  try {
    ...
    await execa.command('git checkout --orphan gh-page')
    console.log(firstLog)
    await execa.command('npm run build', { stdio: 'inherit' })
    await execa.command('git --work-tree dist add --all')
    await execa.command('git --work-tree dist commit -m "gh-page"')
    console.log(secondLog)
    await execa.command('git push origin HEAD:gh-page --force', { stdio: 'inherit' })
    await execa.command('rm -r dist')
    await execa.command(`git checkout -f ${currentBranch}`)
    await execa.command('git branch -D gh-page')
